...
def log_response(func):
    def _log_response(*args, **kwargs) -> requests.Response:
        ...
        body = json.dumps(response.json(), indent=2) if JsonUtils.is_json(response.text) else (
            response.text)
        Logger.info(f"Response status code='{response.status_code}', elapsed_time: "
                    f"'{response.elapsed}' \n{body}\n")
        return response
...
class ApiUtils:
    ...
    def post(self, endpoint_url, **kwargs):
        response = self.session.post(self.url + endpoint_url, **kwargs)
        return response
